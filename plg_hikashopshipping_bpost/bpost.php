...
";
	...
	function _closePopup(){
		...
		$js = "
window.hikashop.ready(function(){
	window.parent.SHM.closePopup();
});
";
		$doc->addScriptDeclaration("\n<!--\n".$js."\n//-->\n");
	}

	function _getCacheKey(&$order){
	...
	function _displayPopup(&$order, &$rate, $check=true){
		...
		foreach($order->products as $product){
			$vars['orderLine'][] = $product->product_name.'|'.$product->cart_product_quantity;
		}
		...
		foreach($vars as $k => $v){
			if(is_array($v)){
				foreach($v as $line){
					$parameters .= $k.': \''.str_replace("'","\'",$line).'\','."\n";
				}
			}else{
				$parameters .= $k.': \''.str_replace("'","\'",$v).'\','."\n";
			}
		}
...
";
	...
	function onShippingConfiguration(&$element) {
		$this->aupost2 = JRequest::getCmd('name','bpost');
